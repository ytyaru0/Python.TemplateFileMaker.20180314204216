...
`do`を使う予定だったがbashの予約語だった。

```sh
bash: 予期しないトークン `do' 周辺に構文エラーがあります
```

* bashで語を入力後TAB押下で候補がでる
    * `do`: `for in do done`
    * bash予約語
        * http://itdoc.hitachi.co.jp/manuals/3020/30203S3530/JPAS0121.HTM
        * `!　[[　{　}　case　do　done　elif　else　esac　fi　for　function　if　in　select　then　time　until　while`
    * コマンド
        * 環境変数のPATHが通ったコマンドとも重複してはならない
            * 多数ある（`set`, `unset`, `export`, `seq`, ...）

回避する方法は以下の通り。だが、楽に実行するという本懐を遂げられない。

方法|コマンド
----|--------
* 絶対パス入力|`$ ~/root/script/sh/_command/do`
* 相対パス入力|`$ cd ~/root/script/sh/_command` `$ ./do`
* 文字列として入力|`$ "do"`
* 大文字と組合せて入力|`$ Do`, `$ dO`, `$ DO` (大文字小文字の区別をなくしたら被る)

`Do`, `DO` が最もマシか。だが`do`と入力したら例のエラーが出るのが混乱を誘う。他のコマンドとの兼ね合いもあり、すべて小文字で統一したい。

bashの予約語が使えないとなると、将来`try`も使えなくなるかもしれない。

`do`をやめて最初に想定していた`y`(yaru)を使うか。
...
`"do"`のようにリテラルで囲えば予約語でも使える。これを利用し、別名定義してはどうか。

complete, compgen, でのコマンド名と一致させねば候補表示ができないため、微妙。面倒だがファイル、ディレクトリ、クラス、などの各種名前を変更したほうがいいか。
